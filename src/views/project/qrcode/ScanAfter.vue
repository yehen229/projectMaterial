...
const getprojectinfo = async () => {
  ...
  if (response.code == 200) {
    ...
    project.value.createDatetime = formatDateToYMD(
      project.value.createDatetime
    );
...
const getmaterialClass = async () => {
  ...
  if (response.code == 200) {
    let materialClassifyDivision =
      response.data.materialClassifySectionView.materialClassifyDivision.name;
    let materialClassifyGroup =
      response.data.materialClassifySectionView.materialClassifyGroup.name;
    let materialClassifySection =
      response.data.materialClassifySectionView.materialClassifySection.name;
    materialClass.value =
...
const getprojectTime = async () => {
  let response = await serverGetHistoryViewPageByProjectId(
    project.value.id,
    1,
    10000
  );
...
const getprojectFile = async () => {
  ...
  if (response.code == 200) {
    projectFile.value =
      response.data[0].projectMaterialVerificationDocumentFileList;
    projectid.value =
      response.data[0].buyMaterialView.useMaterialView.projectMaterialView.projectMaterial.projectId;
...
const getView = async (item, path: string) => {
  console.log(item);
  const ret = await getBlobPartdata(projectid.value, item.id, path);
  const blob = await getOnlyBlob(projectid.value, item.id, path);
  //   获取尾数的类型
  ...
  if (
    ...
    ElMessageBox.confirm("该文件类型不支持在线预览，是否选择下载", {
...
const processengineeringfileName = (
  filename: string,
  user,
  index,
  filetype
  ...
  if (filetype == 0) {
    ...
    let name =
      user.realName + "_" + user.tel + "附件" + index + "." + extension;
    return name;
  }
};
    ...
    <el-collapse v-model="actives">
      ...
      <el-collapse-item title="材料订购信息" name="2" v-model="activeNames">
        <el-descriptions style="margin-top: 1%" title="" border>
          ...
          <el-descriptions-item label="品牌名字">{{
            brandInfo.name
          }}</el-descriptions-item>
          ...
          <el-descriptions-item label="编号">{{
            material.materialInfo.itemMark
          }}</el-descriptions-item>
          <el-descriptions-item label="材料数量">{{
            material.materialCount
          }}</el-descriptions-item>

          <el-descriptions-item label="材料数量单位">{{
            material.materialUnit
          }}</el-descriptions-item>

          <el-descriptions-item label="材料批次">{{
            material.batch
          }}</el-descriptions-item>

          <el-descriptions-item label="存放位置">{{
            material.materialInfo.location
          }}</el-descriptions-item>

          <el-descriptions-item label="颜色">
            {{ material.materialInfo.color }}
          </el-descriptions-item>
          <el-descriptions-item label="材料材质">{{
            material.materialInfo.material
          }}</el-descriptions-item>

          <el-descriptions-item label="防火等级">{{
            material.materialInfo.fireRating
          }}</el-descriptions-item>
          <el-descriptions-item label="规格">{{
            material.materialInfo.dimension
          }}</el-descriptions-item>

          <el-descriptions-item label="施工要求">{{
            material.materialInfo.installation
          }}</el-descriptions-item>
          <el-descriptions-item
            :rowspan="5"
            :width="140"
          ...
          <el-descriptions-item label="工程材料">
            <div>
              <div v-for="(item, index) in projectFile" :key="index">
                <div
                  @click="getView(item, item.filePath)"
                  style="color: #409eff; margin-top: 6px"
                >
                  {{
                    processengineeringfileName(
                      item.filePath,
                      userInfo,
                      index,
                      item.fileType
            ...
            <el-card>
              <el-descriptions :column="1" border>
                <el-descriptions-item label="阶段">
                  <el-tag type="primary">
                    {{ item.projectOpHistory.stepPhase }}</el-tag
                  >
                </el-descriptions-item>
                <el-descriptions-item label="操作">{{
                  item.projectOpHistory.stepDescription
                }}</el-descriptions-item>
