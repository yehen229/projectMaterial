...
import {
  servergetunpassbeforezongbao, servergetunpassjianli, servergetunpassreviewbefore_zongbao,
  servergetunpassreviewbyprojectidmaterialid_companyid, servergetunpassreviewjianli
...
const getdesignUnpassData = async () => {
  try {
    // 调用 API 获取项目列表
    const ret = await servergetunpassjianli(pageNo.value, pageSize.value);
...
//   调取接口 获取对应的材料审核记录
  ...
  try {
    // 调用 API 获取项目列表
    const ret = await servergetunpassreviewjianli(projectId, materialId);
            ...
            <el-collapse @change="handleCollapseChange(scope.$index)">
              <el-collapse-item title="审核详情" name="1">
                <el-timeline style="max-width: 600px">
                  <el-timeline-item :timestamp="formatDate(item.projectMaterialRetest.reviewDatetime)"
                                    placement="top"
