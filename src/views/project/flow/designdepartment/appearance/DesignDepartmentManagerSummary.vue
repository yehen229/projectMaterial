...
import {
  ...
  serverSubmitAffectAppearanceReviewOfEngineeringDepartmentManagerSummary,
  serverSubmitAffectAppearanceReviewOfDesignDepartmentManagerSummary,
...
import {
  ...
  serverGetStatisticsOfDesignDepartmentProjectAppearanceReviewUserViewByTaskId,
...
const projectReviewUserViewList = ref<IServerProjectAppearanceReviewUserView[]>(
  []
);
...
const getStatisticsOfProjectReviewUserFromServer = async () => {
  if (projectUserTask.value && projectUserTask.value.taskId) {
    const ret =
      await serverGetStatisticsOfDesignDepartmentProjectAppearanceReviewUserViewByTaskId(
...
const submitToServerManagerDirect = async (userId: string) => {
  ...
  //直接审核
  const projectAppearanceReviewManagerSummaryForm: IServerProjectAppearanceReviewManagerSummaryForm =
    {
  ...
  // 调用API
  const response =
    await serverSubmitAffectAppearanceReviewOfDesignDepartmentManagerSummary(
...
const onSetAllEmployeeFilesAsManagerFiles = (
  _projectReviewUserViewList: IServerProjectAppearanceReviewUserView[]
) => {
