...
const submitToServerManagerDirect = async (userId: string) => {
  //审核
  const projectReviewEmployeeForm: IServerProjectReviewEmployeeForm = {
    projectReviewUser: {
      id: "", //id,主键
  ...
  console.log(projectReviewEmployeeForm);

  // 调用API
  const response = await serverSubmitProjectMaterialReviewOfEmployee(
