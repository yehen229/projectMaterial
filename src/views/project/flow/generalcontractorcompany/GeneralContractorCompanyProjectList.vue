...
import {
  ...
  serverGetCurrentLoginUserOfGeneralContractorCompanyNotEndedProjectPageView,
...
const fetchTableData = async () => {
  try {
    // 调用 API 获取项目列表
    const ret =
      await serverGetCurrentLoginUserOfGeneralContractorCompanyNotEndedProjectPageView(
        pageNo.value,
        pageSize.value
      );
 ...
 * @param projectItem
 */
const onSelectBrandAndProjectMaterialRequestClick = (
  index: number,
  projectItem: IServerProjectView
) => {
  router.push({
    path: `/general-contractor-company-new-task-of-brand-selection/${projectItem.project.id}`,
  });
};
const onBuyProjectMaterialRequestClick = (
  index: number,
  projectItem: IServerProjectMaterialView
) => {
  router.push({
    path: `/general-contractor-company-new-task-of-buy-material/${projectItem.project.id}`,
  });
};
const onProjectMaterialAcceptanceRequestClick = (
  index: number,
  projectItem: IServerProjectMaterialView
) => {
  router.push({
    path: `/general-contractor-company-new-task-of-project-material-acceptance/${projectItem.project.id}`,
  });
};
        ...
        <el-table-column label="操作">
          <template #default="scope">
            ...
            <el-button
              size="small"
              @click="
                onSelectBrandAndProjectMaterialRequestClick(
                  scope.$index,
                  scope.row
                )
              "
            ...
            <el-button
              size="small"
              @click="onBuyProjectMaterialRequestClick(scope.$index, scope.row)"
            >
              物料订购
            </el-button>
            <el-button
              size="small"
              @click="onProjectMaterialAcceptanceRequestClick(scope.$index, scope.row)"
            >
              物料验收
            </el-button>
          </template>
