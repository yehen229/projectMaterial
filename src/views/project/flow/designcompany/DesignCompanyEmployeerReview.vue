...
import {
  ...
  serverSubmitAffectAppearanceReviewOfEngineeringDepartmentEmployee,
  serverSubmitAffectAppearanceReviewOfDesignCompany,
...
const submitProcess = async () => {
  ...
  if (fileListUploadNum.value == 0) {
    ElMessageBox.confirm("附件为空，是否继续提交？", "提示", {
      ...
      .then(async () => {
        //继续提交
        await submitToServerReviewOfEmployee(userId);
      })
      .catch((action: Action) => {
        return;
      });
...
const submitToServerReviewOfEmployee = async (userId: string) => {
  const projectAppearanceReviewEmployeeForm: IServerProjectAppearanceReviewEmployeeForm =
    {
  ...
  console.log(projectAppearanceReviewEmployeeForm);

  // 调用API
  const response = await serverSubmitAffectAppearanceReviewOfDesignCompany(
  ...
  <div class="tab-container">
    ...
    <div v-if="projectUserTask">
      <div class="review-container container">
        ...
        <el-alert title="通过后需要设计部审核，不通过总包单位重新申请" type="info" show-icon style="margin-top: 5px"/>
