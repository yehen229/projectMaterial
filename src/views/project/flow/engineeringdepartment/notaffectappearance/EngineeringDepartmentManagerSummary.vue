...
import {
  ...
  serverSubmitNotAffectAppearanceReviewOfEngineeringDepartmentManagerSummary,
...
const getStatisticsOfProjectReviewUserFromServer = async () => {
  if (projectUserTask.value && projectUserTask.value.taskId) {
    const ret =
      await serverGetStatisticsOfProjectAppearanceReviewUserViewByTaskId(
...
const submitToServerManagerDirect = async (userId: string) => {
  ...
  // 调用API
  const response =
    await serverSubmitNotAffectAppearanceReviewOfEngineeringDepartmentManagerSummary(
