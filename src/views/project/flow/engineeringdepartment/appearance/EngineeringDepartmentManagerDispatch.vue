...
onBeforeRouteUpdate(async (to) => {
  ...
  if (
    ...
    "Activity_Affect_Appearance_Engineering_Department_Distribution_Review"
  )
...
const submitToServerManagerDistributeToEmployees = async (userId: string) => {
  ...
  // 调用API
  const response =
    await serverSubmitAffectAppearanceReviewOfEngineeringDepartmentManagerDispatchToEmployee(
...
const submitToServerManagerDirect = async (userId: string) => {
  ...
  // 调用API
  const response =
    await serverSubmitAffectAppearanceReviewOfEngineeringDepartmentManagerDirect(
