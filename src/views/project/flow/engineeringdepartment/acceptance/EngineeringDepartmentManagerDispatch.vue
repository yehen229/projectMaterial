...
import {
  ...
  serverSubmitProjectMaterialAcceptanceReviewOfEngineeringDepartmentManagerDirect,
  serverSubmitProjectMaterialAcceptanceReviewOfEngineeringDepartmentManagerDispatchToEmployee,
...
onBeforeRouteUpdate(async (to) => {
  ...
  if (
    ...
    "Activity_Engineering_Department_Project_Material_Acceptance_Distribution_Review"
  )
...
const handleRadioChange = async () => {
  ...
  if (radio.value == 0 && employeeList.value.length == 0) {
    //分发审核,显示项目员工名单
    const ret = await serverGetEmployeeUserOfEngineeringDepartment(
      projectId.value
    );
    if (ret && ret.code == 200) {
...
const submitToServerManagerDistributeToEmployees = async (userId: string) => {
  //分发审核
  const projectReviewDispatchForm: IServerProjectMaterialAcceptanceReviewDispatchForm =
    {
      projectId: projectId.value,
      taskId: taskId.value,
      projectMaterialAcceptanceReviewMode: {
        id: "", //id,主键
        projectMaterialAcceptanceBatchId: "", //t_project_id,外键,	t_project_id<-表t_project.id
  ...
  // 调用API
  const response =
    await serverSubmitProjectMaterialAcceptanceReviewOfEngineeringDepartmentManagerDispatchToEmployee(
...
const submitToServerManagerDirect = async (userId: string) => {
  //直接审核
  const projectAppearanceReviewManagerDirectForm: IServerProjectMaterialAcceptanceReviewManagerDirectForm =
    {
      projectMaterialAcceptanceReviewMode: {
        id: "", //id,主键
        projectMaterialAcceptanceBatchId: "", //t_use_material_brand_select_id
  ...
  // 调用API
  const response =
    await serverSubmitProjectMaterialAcceptanceReviewOfEngineeringDepartmentManagerDirect(
