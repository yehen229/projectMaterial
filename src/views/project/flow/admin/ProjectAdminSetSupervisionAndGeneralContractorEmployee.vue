...
import {
  ...
  serverSubmitAdminSetSuperVisionAndGeneralContractorEmployee,
...
const onSubmitProjectUserTask = async () => {
  if (projectUserTask && projectUserTask.value?.projectView) {
    const ret =
      await serverSubmitAdminSetSuperVisionAndGeneralContractorEmployee(
...
<template>
  ...
  <div v-if="projectUserTask">
    ...
    <div class="admin-set-employee-container container">
      <div style="font: 1.2em sans-serif; margin: 10px; margin-bottom: 20px">
        设置监理单位与总包单位管理员工
      </div>
