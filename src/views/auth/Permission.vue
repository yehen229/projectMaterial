...
import {
  serverPermissionAdd, //增加
  serverPermissionDelete, //删除
  serverPermissionUpdate, //修改
  serverGetPermissionPage, //得到指定页面数据
  serverGetPermissionPageByName, //根据名称得到指定页面数据
} from "@/server/auth/SysPermission";

//服务器返回到前端的类型
import { IServerResponseData, IServerSysRole, IServerPage } from "@/server/ServerType";
...
const onRowDeleteButtonClick = async (index: number, row: IServerSysRole) => {
  ...
  ElMessageBox.confirm("是否真的删除数据？", "警告", {
    ...
    .then(async () => {
      await serverPermissionDelete(row.id);
      await getPermissionsFromSever();
      ElMessage({
        type: "success",
        message: "完成删除",
...
const submitNewForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      let ret = await serverPermissionAdd(form.name, form.note);
      if (ret != null) {
        await getPermissionsFromSever();
      }
...
const submitUpdateForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      let ret = await serverPermissionUpdate(form.id, form.name, form.note);
      if (ret != null) {
        await getPermissionsFromSever();
      }
...
const getPermissionsFromSever = async () => {
...
const onSearchClick = async () => {
  ...
  if (search) {
    pageNo.value = 1;
    let ret = await serverGetPermissionPageByName(
      searchText.value,
      pageNo.value,
      pageSize.value
    );
  ...
  } else await getPermissionsFromSever();
};
...
<template>
  <!--新增对话框-->
  <el-dialog v-model="dialogFormNewVisible" title="增加权限" draggable>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="submitNewForm(ruleFormRef)"> 确定 </el-button>
      </span>
    </template>
  </el-dialog>

  <!--修改对话框-->
  <el-dialog v-model="dialogFormUpdateVisible" title="修改权限" draggable>
    <el-form ref="ruleFormRef" :model="form" :rules="rules">
      <el-form-item label="权限名称" :label-width="formLabelWidth" prop="name">
        <el-input v-model="form.name" autocomplete="off" />
      </el-form-item>
  ...
  <el-page-header @back="goBack" style="margin-bottom: 20px">
    <template #content>
      <span class="text-large font-600 mr-3">权限管理</span>
    </template>
  ...
  <div class="tab-container">
    <div class="top-toolbar">
      <!--新增按钮-->
      <el-button :icon="Plus" type="primary" @click="onNewButtonClick"> 新增 </el-button>
