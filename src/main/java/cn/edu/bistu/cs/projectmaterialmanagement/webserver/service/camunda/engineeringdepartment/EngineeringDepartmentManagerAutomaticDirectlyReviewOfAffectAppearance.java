 ...
 */
@Service
public class EngineeringDepartmentManagerAutomaticDirectlyReviewOfAffectAppearance implements JavaDelegate {
    ...
    public EngineeringDepartmentManagerAutomaticDirectlyReviewOfAffectAppearance(IProjectOpHistoryBusiness projectHistoryBusiness,
    ...
    public void execute(DelegateExecution delegateExecution) throws Exception {
        ...
        projectHistoryBusiness.addAffectAppearanceReviewEngineeringDepartmentManagerDirectly(businessId,
                                                                                             user.getId(),
                                                                                             "工程部审核影响外观的物料使用申请",
                                                                                             "项目经理直接对影响外观的品牌、物料申请等进行审核",
                                                                                             ""
        );
        Log log = new Log(user.getId(), businessId, "项目经理直接对影响外观的品牌、物料申请等进行审核", 0 , new Date());
        logService.add(log);
