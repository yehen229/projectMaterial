...
public List<Stastisprojectidandcount> getzongbaounpassorpassList(int result) {
    return jdbcTemplate.query("""
           ...
           FROM
               t_project_material
           JOIN t_use_material 
               ON t_project_material.id = t_use_material.t_project_material_id
           JOIN t_project_appearance_review_mode 
               ON t_use_material.t_use_material_brand_select_id = t_project_appearance_review_mode.t_use_material_brand_select_id
           JOIN t_project_appearance_review 
               ON t_project_appearance_review_mode.id = t_project_appearance_review.t_project_appearance_review_mode_id
           JOIN t_project_appearance_review_user 
               ON t_project_appearance_review.id = t_project_appearance_review_user.t_project_appearance_review_id
           WHERE
    ...
    public Integer getzongbaounpassorpassCount(String projectid, int result) {
        ...
        JOIN t_project_appearance_review_user 
            ON t_project_appearance_review.id = t_project_appearance_review_user.t_project_appearance_review_id
        WHERE t_project_appearance_review.review_result = ?
...
public List<Stastisprojectidandcount> getjianliunpassorpassList(int result) {
    return jdbcTemplate.query("""
        SELECT 
            t_buy_material.t_use_material_id,
            t_project_material_retest_batch.t_project_id,
            MAX(t_project_material_retest.review_datetime) AS latest_review_time
        FROM t_buy_material
        JOIN t_project_material_retest_batch 
            ON t_buy_material.t_buy_material_batch_id = t_project_material_retest_batch.t_buy_material_batch_id
        JOIN t_project_material_retest 
            ON t_project_material_retest_batch.id = t_project_material_retest.t_project_material_retest_batch_id
           AND t_buy_material.id = t_project_material_retest.t_buy_material_id
        WHERE t_project_material_retest.review_result = ?
        GROUP BY 
            t_buy_material.t_use_material_id,
            t_project_material_retest_batch.t_project_id
...
//监理与工程部
    ...
    public List<Unpassmaterialmessage> getUnpassmaterialmessage() {
        ...
        SELECT COUNT(*) 
        FROM t_project_material
        JOIN t_project_review_mode 
            ON t_project_material.t_company_id = t_project_review_mode.t_company_id
           AND t_project_material.t_project_id = t_project_review_mode.t_project_id
        JOIN t_project_review 
            ON t_project_review.t_project_review_mode_id = t_project_review_mode.id
        JOIN t_project_review_user 
            ON t_project_review.id = t_project_review_user.t_project_review_id
        WHERE t_project_review.review_result = 2
        """, Integer.class);
    ...
    public List<Unpassonlymaterial> getUnpassOnlymaterial() {
        ...
        SELECT COUNT(*) FROM (
            ...
            JOIN t_project_review_mode 
               ...
               AND t_project_material.t_project_id = t_project_review_mode.t_project_id
            JOIN t_project_review 
                ON t_project_review_mode.id = t_project_review.t_project_review_mode_id
            JOIN t_project_review_user 
                ON t_project_review.id = t_project_review_user.t_project_review_id
            WHERE t_project_review.review_result = 2
        ) AS subquery
    ...
    private List<Unpassonlymaterial> getPageQuery(int pageNo,
        ...
        return jdbcTemplate.query("""
            ...
            WHERE t_project_review.review_result = 2
            LIMIT ? OFFSET ?
            """, new StatisticalanalysisMaterilaMapper(), pageSize, pageNo * pageSize);
    }
    ...
    public int getunpassbeforezongbaoCount() {
        ...
        SELECT COUNT(*) FROM (
            ...
            JOIN t_project_appearance_review_user 
                ON t_project_appearance_review.id = t_project_appearance_review_user.t_project_appearance_review_id
            WHERE t_project_appearance_review.review_result = 2
        ) AS subquery
        """, Integer.class);
    ...
    private List<Unpassonlymaterial> getunpassbeforezongbaoPageQuery(int pageNo,
        ...
        ORDER BY latest_review_time DESC
        LIMIT ? OFFSET ?
        """, new StatisticalanalysisgetunpassbeforezongbaoMapper(), pageSize, pageNo * pageSize);
    }
    ...
    private List<Unpassonlymaterial> getUnpassReviewJianliPageQuery(int pageNo,
        ...
        JOIN t_project_material_retest 
            ON t_project_material_retest_batch.id = t_project_material_retest.t_project_material_retest_batch_id
           AND t_buy_material.id = t_project_material_retest.t_buy_material_id
        WHERE t_project_material_retest.review_result = 2
        GROUP BY 
            t_buy_material.t_use_material_id,
            t_project_material_retest_batch.t_project_id
        ORDER BY latest_review_time DESC
        LIMIT ? OFFSET ?
        """, new StatisticalanalysisgetUnpassReviewJianliMapper(), pageSize, pageNo * pageSize);
    }
    ...
    public int getUnpassReviewJianliCount() {
        ...
        SELECT COUNT(*) FROM (
            ...
            JOIN t_project_material_retest 
               ...
               AND t_buy_material.id = t_project_material_retest.t_buy_material_id
            WHERE t_project_material_retest.review_result = 2
            GROUP BY t_buy_material.t_use_material_id, t_project_material_retest_batch.t_project_id
        ) AS sub
        """, Integer.class);
        return i == null ? 0 : i;
    }
    ...
    public int getUnpassReviewJianliAndGongchengbuliCount() {
        ...
        SELECT COUNT(*) FROM (
            SELECT 
                t_project_material_acceptance_batch.t_project_id, 
                t_project_material_acceptance.t_project_material_id
            FROM t_project_material_acceptance
            JOIN t_project_material_acceptance_batch 
                ON t_project_material_acceptance.t_project_material_acceptance_batch_id = t_project_material_acceptance_batch.id
            ...
            JOIN t_project_material_acceptance_review 
            ...
            JOIN t_project_material_acceptance_review_user 
                ON t_project_material_acceptance_review_user.t_project_material_acceptance_review_id = t_project_material_acceptance_review.id
            WHERE t_project_material_acceptance_review_user.review_result = 2
            GROUP BY t_project_material_acceptance_batch.t_project_id, t_project_material_acceptance.t_project_material_id
        ) AS sub
        """, Integer.class);
        return i == null ? 0 : i;
    ...
    private List<Unpassonlymaterial> getUnpassReviewJianliAndGongchengbuPageQuery(int pageNo,
        ...
        ORDER BY latest_review_time DESC
        LIMIT ? OFFSET ?
        """, new StatisticalanalysisgetUnpassReviewJianliAndGongchengbuMapper(), pageSize, pageNo * pageSize);
    }
