...
const projectMaterialViewPageData =
  ref<IServerPage<IServerUseMaterialView> | null>(null);
const updateProjectMaterialView = ref<IServerUseMaterialView>();
const projectViewData = ref<IServerProjectView>();
const radioUserType = ref(0);

const multipleTableRef = ref<InstanceType<typeof ElTable>>();
const multipleSelection = ref<IServerUseMaterialView[]>([]);
const toggleSelection = (rows?: IServerUseMaterialView[]) => {
  if (rows) {
    rows.forEach((row) => {
...
const handleSelectionChange = (val: IServerUseMaterialView[]) => {
            ...
            <el-table-column label="品牌" width="100" show-overflow-tooltip>
              <template #default="scope">
                <div style="display: flex; align-items: center">
                  ...
                  <span
                    v-if="scope.row.projectMaterialBrandPublicView != null"
                    >{{
