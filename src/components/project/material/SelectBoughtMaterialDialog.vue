 ...
 */
const getProjectBoughtMaterialViewFromSever = async () => {
  ...
  if (search) {
    ...
    const ret = await serverGetBoughtUseMaterialPageViewByProject(
      props.projectId,
      pageNo.value,
      pageSize.value
    );
...
const onPageCurrentChange = async (value: number) => {
  pageNo.value = value;
  await getProjectBoughtMaterialViewFromSever();
};
            ...
            <el-table-column label="品牌" width="100" show-overflow-tooltip>
              <template #default="scope">
                <div style="display: flex; align-items: center">
                  <!--公共品牌-->
                  <div
                    v-for="bublicBrand in scope.row
                      .projectMaterialBrandPublicViewList"
                  >
                    {{ bublicBrand.brandPublicView.brandView.brand.name }}
                  </div>

                  <!--项目私有品牌-->
                  <div
                    v-for="privateBrand in scope.row
                      .projectMaterialBrandPrivateViewList"
                  >
                    {{ privateBrand.projectBrandView.brandView.brand.name }}
                  </div>
                </div>
              </template>
            </el-table-column>
