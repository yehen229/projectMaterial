...
const projectReviewUserViewData = ref<IServerProjectReviewUserView>();
...
const getProjectReviewUserViewPageFromServer = async () => {
  ...
  if (
    ...
    const ret =
      await serverGetFeedbackOfProjectMaterialReviewedByProjectIdAndTaskId(
        projectUserTask.projectView.project.id,
        projectUserTask.taskId
      );
    console.log(ret);
...
const reviewResult = computed(() => {
  if (
    ...
    projectReviewUserViewData.value.projectReview
  ) {
    if (projectReviewUserViewData.value.projectReview.reviewResult == 2)
      return "不通过";
    if (projectReviewUserViewData.value.projectReview.reviewResult == 1)
      return "通过";
  }
});
...
<template>
  <div
    ...
    v-if="projectReviewUserViewData && projectReviewUserViewData.projectReview"
  >
    ...
    <div class="project-container" v-show="!collapsed">
      <div style="display: flex; margin: 10px; align-items: baseline">
        <div>审核结果：</div>
        <div
          v-if="projectReviewUserViewData?.projectReview.reviewResult == 2 || projectReviewUserViewData?.projectReview.reviewResult == 0"
          style="color: red"
        >
          不通过
        </div>
        <div
          v-else-if="projectReviewUserViewData?.projectReview.reviewResult == 1"
          style="color: green"
        >
          通过
        </div>
      ...
      <div style="display: flex; margin: 10px; align-items: baseline">
        <div>审核意见：</div>
        <div>
          {{ projectReviewUserViewData?.projectReviewUser.reviewContent }}
        </div>
      </div>
      ...
      <div style="display: flex; margin: 10px; align-items: baseline">
        ...
        <div>
          {{
            formatDate(
              projectReviewUserViewData?.projectReviewUser.reviewDatetime
            )
          }}
        </div>
      </div>
      ...
      <div style="display: flex; margin: 10px; align-items: baseline">
        ...
        <div>
          <div
            v-for="(
              item, index
            ) in projectReviewUserViewData?.projectReviewUserFileList"
            :key="index"
