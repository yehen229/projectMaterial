...
import {
  ...
  serverGetPageProjectMaterialAcceptanceReviewUserViewByProjectIdAndTaskId,
...
const projectReviewUserViewPageData =
  ref<IServerPage<IServerProjectMaterialAcceptanceReviewUserView> | null>(null);
...
const getProjectReviewUserViewPageFromServer = async () => {
  ...
  if (
    ...
    const ret = await serverGetPageProjectMaterialAcceptanceReviewUserViewByProjectIdAndTaskId(
      projectUserTask.projectView.project.id,
      projectUserTask.taskId,
      pageNo.value,
      pageSize.value
        ...
        <el-table-column label="审核意见" show-overflow-tooltip>
          <template #default="scope">
            <div style="display: flex; align-items: center">
              <span
                v-if="scope.row.projectMaterialAcceptanceReviewUser.reviewResult > 0"
                >{{ scope.row.projectMaterialAcceptanceReviewUser.reviewContent }}</span
              >
            </div>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div style="display: flex; align-items: center">
              <span
                ...
                >{{
                  formatDate(
                    scope.row.projectMaterialAcceptanceReviewUser.reviewDatetime
                  )
                }}</span
              >
            </div>
