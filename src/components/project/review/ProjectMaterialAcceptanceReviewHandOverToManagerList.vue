...
import {
  serverGetProjectMaterialAcceptanceReviewUserViewListByTaskId,
...
const updateProjectMaterialView = ref<IServerProjectMaterialAcceptanceReviewUserView>();
...
const emit = defineEmits<{
  (
    ...
    projectReviewUserViewList: IServerProjectMaterialAcceptanceReviewUserView[]
  ): void;
}>();
...
const setAllFilesAsManagerFiles = async () => {
  if (
    ...
    const ret = await serverGetProjectMaterialAcceptanceReviewUserViewListByTaskId(
      projectUserTask.projectView.project.id,
      projectUserTask.taskId
    );
    ...
    if (ret && ret.code == 200) {
      ...
      if (ret.data) {
        ret.data.forEach((item) => {
          if (
            item.projectMaterialAcceptanceReviewUserFileList &&
            item.projectMaterialAcceptanceReviewUserFileList.length > 0
          )
...
const onRowShowButtonClick = async (
  index: number,
  row: IServerProjectMaterialAcceptanceReviewUserView
) => {
  console.log(row);
...
const deleteUserReviewFile = (id: string) => {
  projectReviewUserViewListData.value.forEach(
    (item: IServerProjectMaterialAcceptanceReviewUserView) => {
      if (
        item.projectMaterialAcceptanceReviewUserFileList.some(
          (itemFile) => itemFile.id === id
        )
      )
        item.projectMaterialAcceptanceReviewUserFileList =
          item.projectMaterialAcceptanceReviewUserFileList.filter(
            (itemFile) => itemFile.id !== id
          );
    }
  );
  ...
  projectReviewUserViewListData.value.length = 0;
  temp.forEach((item: IServerProjectMaterialAcceptanceReviewUserView) =>
    projectReviewUserViewListData.value.push(item)
  );
        ...
        <el-table-column label="审核结果" width="100" show-overflow-tooltip>
          <template #default="scope">
            <div style="display: flex; align-items: center">
              <el-tag
                type="info"
                v-if="scope.row.projectMaterialAcceptanceReviewUser.reviewResult == 0"
                >尚未审核</el-tag
              >
              <el-tag
                type="success"
                v-else-if="
                  scope.row.projectMaterialAcceptanceReviewUser.reviewResult == 1
                "
                >审核通过</el-tag
              >
              <el-tag
                type="danger"
                v-else-if="
                  scope.row.projectMaterialAcceptanceReviewUser.reviewResult == 2
                "
                >审核不通过</el-tag
              >
            </div>
            ...
            <div style="display: flex">
              <div
                v-for="(item, index) in scope.row
                  .projectMaterialAcceptanceReviewUserFileList"
                :key="index"
                style="
                  display: flex;
          ...
          <template #default="scope">
            <div style="display: flex; align-items: center">
              <span style="margin-left: 10px">{{
                formatDate(scope.row.projectMaterialAcceptanceReviewUser.reviewDatetime)
              }}</span>
            </div>
          </template>
        </el-table-column>
