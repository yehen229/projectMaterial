...
const form = reactive<IServerBrand>({
  id: "", //id,主键
  name: "", //名称
  materialClassifySectionId: props.materialClassifySectionId, //
  position: "国产", //
  deletedAt: new Date(),
});

watch(
  () => props.materialClassifySectionId,
  (newVal, oldVal) => {
    if (newVal) {
      form.materialClassifySectionId = props.materialClassifySectionId;
    }
  }
);
