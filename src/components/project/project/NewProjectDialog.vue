...
const userSelectCompanyDesignList = ref<IServerCompany[]>();
...
const emit = defineEmits<{
  (e: "onDilalogOk", projectF: IServerProjectForm): void;
...
const onOk = () => {
  ...
  if (
    !userSelectCompanyDesignList.value ||
    userSelectCompanyDesignList.value.length == 0
  ) {
    ElMessageBox.alert(
      "没有添加设计单位，请添加设计单位，最多不能超过8个",
      "提示",
      {
        confirmButtonText: "确定",
      }
  ...
  projectFormRef.value?.validate(async (valid) => {
    ...
    if (valid) {
      try {
        ...
        const projectF: IServerProjectForm = {
          project: {
            id: "", // 在后端生成
            userId: userId,
          ...
          companyDesignList: userSelectCompanyDesignList.value || [],
        };

        emit("onDilalogOk", projectF);
      } catch (error) {
...
const dialogDesignCompanyVisible = ref(false); //控制“设计公司对话框”是否显示

const onAddDesignCompany = () => {
  dialogDesignCompanyVisible.value = true;
};
const onDesignCompanyDialogCancel = () => {
  dialogDesignCompanyVisible.value = false;
};
const onDesignCompanyDialogOk = () => {
  dialogDesignCompanyVisible.value = false;
};

const onUserSelectedDesignCompanyChanged = () => {
  if (!companyDesignList.value || !companyDesignList.value.length) return;
  userSelectCompanyDesignList.value = [];
  if (projectForm.companyDesignIds.length > 0) {
    projectForm.companyDesignIds.forEach((id) => {
      userSelectCompanyDesignList.value.push(
        companyDesignList.value.find((company) => company.id === id)
      );
    });
  }
};

const onDeleteDesignCompany = (id: string) => {
  userSelectCompanyDesignList.value = userSelectCompanyDesignList.value.filter(
...
<template>
  <el-dialog
    v-model="dialogDesignCompanyVisible"
    width="500"
    title="请选择设计单位"
    append-to-body
  >
    <el-checkbox-group
      v-model="projectForm.companyDesignIds"
      @change="onUserSelectedDesignCompanyChanged"
    >
      <el-checkbox
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="onDesignCompanyDialogCancel">取消</el-button>
        <el-button type="primary" @click="onDesignCompanyDialogOk()">
          确定
        </el-button>
      </span>
    </template>
    ...
    >
      ...
      <el-form-item label="设计单位" prop="companyDesignId">
        <div style="display: block">
          <el-button @click="onAddDesignCompany" size="small"
            >新增设计单位</el-button
          >
          <div
            v-for="(company, index) in userSelectCompanyDesignList"
            style="display: block"
          >
            {{ company.name
            }}<el-button
              @click="onDeleteDesignCompany(company.id)"
              size="small"
              :icon="Delete"
              type="danger"
              >删除</el-button
