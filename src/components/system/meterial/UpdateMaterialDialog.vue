  ...
  if (isValid) {
    ...
    try {
      const material: IServerMaterial = {
        id: props.materialView.material.id,
      ...
      fileList.value.forEach((item) => {
        materialNewForm.photoIds.push(item.name);
      });
      ...
      const response = await serverMaterialUpdateForm(materialNewForm);
      if (response && response.code === 200) {
        console.log(response.data);
        ElMessage.success("材料修改成功");
      } else {
        console.error("材料修改失败");
      }
